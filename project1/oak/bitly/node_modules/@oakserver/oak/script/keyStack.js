...
async function compare(a, b) {
    ...
    const cryptoKey = await (0, util_js_1.importKey)(key);
    const ah = await (0, util_js_1.sign)(a, cryptoKey);
    const bh = await (0, util_js_1.sign)(b, cryptoKey);
    return (0, deps_js_1.timingSafeEqual)(ah, bh);
}
class KeyStack {
    ...
    constructor(keys) {
        ...
        tslib_1.__classPrivateFieldSet(this, _KeyStack_keys, keys, "f");
    }
    get length() {
    ...
    async sign(data) {
        ...
        return (0, util_js_1.encodeBase64Safe)(await (0, util_js_1.sign)(data, key));
    }
    ...
    async indexOf(data, digest) {
        for (let i = 0; i < tslib_1.__classPrivateFieldGet(this, _KeyStack_keys, "f").length; i++) {
            ...
            if (await compare(digest, (0, util_js_1.encodeBase64Safe)(await (0, util_js_1.sign)(data, cryptoKey)))) {
                return i;
            }
        }
        return -1;
    ...
    [(_KeyStack_cryptoKeys = new WeakMap(), _KeyStack_keys = new WeakMap(), _KeyStack_instances = new WeakSet(), _KeyStack_toCryptoKey = async function _KeyStack_toCryptoKey(key) {
        if (!tslib_1.__classPrivateFieldGet(this, _KeyStack_cryptoKeys, "f").has(key)) {
            tslib_1.__classPrivateFieldGet(this, _KeyStack_cryptoKeys, "f").set(key, await (0, util_js_1.importKey)(key));
        }
