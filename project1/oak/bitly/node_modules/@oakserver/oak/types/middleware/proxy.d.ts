...
export declare type Fetch = (input: dntShim.Request) => Promise<dntShim.Response>;
export declare type ProxyMatchFunction<R extends string, P extends RouteParams<R> = RouteParams<R>, S extends State = Record<string, any>> = (ctx: Context<S> | RouterContext<R, P, S>) => boolean;
export declare type ProxyMapFunction<R extends string, P extends RouteParams<R>> = (path: R, params?: P) => R;
export declare type ProxyHeadersFunction<S extends State> = (ctx: Context<S>) => dntShim.HeadersInit | Promise<dntShim.HeadersInit>;
export declare type ProxyRouterHeadersFunction<R extends string, P extends RouteParams<R>, S extends State> = (ctx: RouterContext<R, P, S>) => dntShim.HeadersInit | Promise<dntShim.HeadersInit>;
